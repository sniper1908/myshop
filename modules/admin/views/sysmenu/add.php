										...
										<form class="form-horizontal" id="formMenu" role="form" method="post" action="<?=Url::makeUrl($this->params['module'].'/'.$this->params['controller'].'/'.$this->params['action'])?>">
															...
															<?php
																if(!empty($this->params['tree'])):
																	foreach($this->params['tree'] as $row):
																		echo '<option value="'.$row['id'].'">|-'.str_repeat('--', $row['level']).$row['menu_name'].'</option>';
																	endforeach;
																endif;
															?>
														</select>
														...
														<select id="routeController" class="col-xs-10 col-sm-5" name="menu[route_controller]">
															...
															<?php
																foreach($allController as $row):
																	echo '<option value="'.$row['route_controller'].'">'.$row['route_controller'].'</option>';
																endforeach;
															?>
														</select>
													</div>
														...
														<select id="routeControllerAction" class="col-xs-10 col-sm-5" name="menu[sys_route_id]">
															...
															<?php
																foreach($allControllerAction as $row):
																	echo '<option value="'.$row['id'].'">'.$row['route_url'].'</option>';
																endforeach;
															?>
														</select>
													</div>
											...
											<div class="form-group">
												...
												<div class="col-sm-9">
													<div class="clearfix">
														<input type="text" name="menu[order_num]" class="input-mini" id="spinner3" data-value=0 />
													</div>
												</div>
											</div>
											...
											<div class="form-group">
												...
												<div class="col-sm-9">
													<div class="clearfix">
														<textarea class="col-xs-10 col-sm-5" name="menu[json_data]"></textarea>
													</div>
												</div>
											</div>
