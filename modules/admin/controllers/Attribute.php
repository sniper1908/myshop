 ...
 * @apply 	>=PHP5.4 适用版本
 * @date 	2018/02/04 11:03:04
 * @name 	网站后台文件 - 控制器 - 商品类型属性列表
 */
namespace modules\admin\controllers;

use classes\Web;
use modules\admin\classes\BaseController;
use common\models\AttributeModel;
...
class Attribute extends BaseController
{
	...
	public function indexAction(){
		...
		$this->params['page_title'] = '商品类型属性列表';
		$this->have_script = 1;
		$goods_type_id = (int)$_GET['goods_type_id'];
		if (!$goods_type_id) {
			# 没有商品类型ID，返回上一页
			$this->redirectMsg('没有商品类型ID',['goodsType/index']);
			exit;
		}
		$this->params['urlParamsStr'] = '&goods_type_id='.$goods_type_id;
		$select = [];
		$select['where'] = "`cate_id`='".$goods_type_id."'";
		$aList = $this->model->select($select);
	...
	public function addAction(){
		...
		$goods_type_id = (int)$_GET['goods_type_id'];
		$this->params['goods_type_id'] = $goods_type_id;
		$this->params['urlParamsStr'] = '&goods_type_id='.$goods_type_id;
		// 获取商品类型列表
		$this->params['typeList'] = $this->model->getTypeList();
		if ($this->model->load($_POST) && $this->model->save()) {
		// if($this->model->load($_POST)){
			$this->redirectMsg('添加成功',['index','goods_type_id'=>$goods_type_id],-1);
			exit;
		}
